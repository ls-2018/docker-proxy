...
func Load(ctx context.Context, opt cfg.Options, successCallBack func()) {
	...
	obj := tc_dns_parseObjects{}
	err := loadTc_dns_parseObjects(&obj, &ebpf.CollectionOptions{
	...
	tcx, err := link.AttachTCX(link.TCXOptions{
		Interface: index,
		Program:   obj.tc_dns_parsePrograms.ParseDnsA,
...
func getKey(host string) tc_dns_parseDnsQuestion {
	queryKey := tc_dns_parseDnsQuestion{
		RecordType: 1,
		Class:      1,
	}
