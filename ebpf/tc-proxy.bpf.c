...
long redirect_proxy(struct __sk_buff *skb, struct iphdr *ip, struct tcphdr *tcp) {
    ...
    __u32 pkey = 0;
    struct proxy_redirect_config *hook_eth = bpf_map_lookup_elem(&redirect_map, &pkey);
    if (hook_eth == NULL) {
        return TC_ACT_OK;
    }

    // 避免环路
    if (skb->ifindex != hook_eth->ifindex) {
        return TC_ACT_OK;
    }
...
int proxy_ingress(struct __sk_buff *skb) {
    bpf_printk("proxy_ingress");
    return TC_ACT_OK;
}

SEC("classifier/egress")
int proxy_egress(struct __sk_buff *skb) {
    bpf_printk("proxy_egress");
    void *data = (void *)(long)skb->data;
    void *data_end = (void *)(long)skb->data_end;
    // check ethernet header
    struct ethhdr *eth = data;
